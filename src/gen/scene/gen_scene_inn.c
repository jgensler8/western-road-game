...
static void process_input(void) {
    ...
    case 4:
        ...
        break;
    case 5:
        if(code_ready != 0) { INCR(INN_VISITS); if(QUEST_NONE != QUEST_NONE) { default_state.quest = QUEST_NONE; } code_ready = 0; }
        if(1) {
            if(0){
                progress = 254;
                progress_changed = 1;
...
static void render(const struct SceneRenderOptions* options) {
    if(options->swapped){ character_init(CHARACTER_MODEL_STORE_OWNER,1,1); sound_play_song(SONG_NONE); progress = 0; progress_changed = 1; code_ready = 1; }
